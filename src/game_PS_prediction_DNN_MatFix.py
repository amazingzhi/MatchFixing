...
splits = KFold(n_splits=k,shuffle=shuffle,random_state=random_state)
    # model parameters
learning_rates=[1e-4, 1e-3, 1e-2]  #1e-3, 1e-2
batch_sizes=[16, 32, 64]  # , 32, 64
weight_decays=[1e-4, 1e-6]  # , 1e-6
nodes_propotions=[1]  # , 2, 3
denominator_of_input=len(nodes_propotions)
num_layers = ['model_params2', 'model_params3', 'model_params5', 'model_params8']  # 'model_params1', 'model_params2', 'model_params3',
    # tensorboard
tb = SummaryWriter()
    #read data parameters
train_and_test = True  # True
data_pre = 'feature_selected'  # 'feature_selected', 'PCA'
train_path_ps = '../data/final/final_game_features_ps.csv'  # '../data/final/ps_pca.csv', '../data/final/final_game_features_ps.csv'
predictions_dir = '../predictions/predictions_20_22_test_game/all_before_20'
models_dir = '../models/models_20_22_game/all_before_20/DNN'
accuracies_dir = '../accuracies/accuracies_20_22_game/all_before_20'
runs_dir = '../models/models_20_22_game/all_before_20/DNN/runs'
checkpoint_dir = '../models/models_20_22_game/all_before_20/DNN/checkpoint'
year_to_be_test = '2022'  # the year to be test dataset.
...
df_mat_pred_train = df_mat_pred[~df_mat_pred['GAME_DATE_EST'].str.contains('2020|2021|2022')]
...
def main():
    ...
    for k,v in datasets.items():
        ...
        test_loader_test = DataLoader(test, batch_size=1, shuffle=False, drop_last=True)
        test_loader_train = DataLoader(train, batch_size=1, shuffle=False, drop_last=True)
        ...
        predictions_train, _ = best_opt.evaluate(
            test_loader_train,
            model_ori=best_model,
            optimizer_ori=best_optimizer,
            best_model_path=best_model_path,
        ...
        if k == 'home':
            df_home_mat_pred_train = df_mat_pred[df_mat_pred['loc'] == 1]
            df_home_mat_pred_train[f'{target_col}_pred'] = predictions_train
            df_home_mat_pred = df_mat_pred[df_mat_pred['loc'] == 1]
            df_home_mat_pred[f'{target_col}_pred'] = predictions
        else:
            df_away_mat_pred_train = df_mat_pred[df_mat_pred['loc'] == 0]
            df_away_mat_pred_train[f'{target_col}_pred'] = predictions_train
    ...
    df_pred_train = pd.concat([df_home_mat_pred_train, df_away_mat_pred_train])
    df_pred_train = df_pred_train.sort_index(ascending=True)
    ...
    resultCSVPath = f'{predictions_dir}/{data_pre}_DNN_{transform}_prediction_train.csv'
    df_pred_train.to_csv(resultCSVPath, index=False, na_rep=0)
